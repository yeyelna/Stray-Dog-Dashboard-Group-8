...
# =========================
# CSS: SEPARATE CARDS WITH DISTINCT BORDERS
# =========================
st.markdown(
    f"""
<style>
/* 1. Global Background (Clean Beige, No Border) */
.stApp {{
...
/* This targets ONLY the st.container(border=True) elements */
/* We make them White, with a THICK GREY BORDER and Shadow */
[data-testid="stVerticalBlockBorderWrapper"] {{
    background-color: #ffffff !important;
    border: 2px solid #64748b !important; /* Visible Slate Grey Border */
...
/* 3. INNER SCROLL AREA (No Border) */
/* If a bordered container is INSIDE another (like the scroll box in Active Alerts), hide its border */
...
/* 5. Header Title (No Border, just text) */
.header-container {{
    margin-bottom: 20px;
    padding: 10px 0;
    border-bottom: 2px solid #e2e8f0;
}}
.big-title {{ font-size: 32px; font-weight: 900; }}
...
/* 7. Thumbnails & Badges */
.thumb {{ border-radius: 8px; overflow: hidden; border: 1px solid #cbd5e1; }}
...
st.markdown(
    ...
    <div class="header-container">
        ...
        <div style="color:#64748b; font-size:16px;">Real-Time AI Detection Monitoring</div>
    </div>
    """,
    unsafe_allow_html=True,
)
...
# =========================
# Using gap="medium" + explicit st.container(border=True) for each
...
with k1:
    with st.container(border=True): # <--- SEPARATE BORDER HERE
        ...
        <div style="display:flex; justify-content:space-between; align-items:center;">
            ...
            <div style="font-weight:bold; color:{'#b91c1c' if new_today>0 else '#16a34a'};">+0%</div>
        </div>
...
with k2:
    with st.container(border=True): # <--- SEPARATE BORDER HERE
        ...
        <div style="display:flex; justify-content:space-between; align-items:center;">
            <div style="font-size:28px;">📊</div>
            <div style="font-weight:bold; color:#1e40af;">+0%</div>
        </div>
        <div style="font-size:42px; font-weight:900; margin-top:5px;">{dogs_today}</div>
        <div style="font-weight:bold; color:#64748b; font-size:14px;">TOTAL DOGS DETECTED</div>
        """, unsafe_allow_html=True)
...
with k3:
    with st.container(border=True): # <--- SEPARATE BORDER HERE
        ...
        <div style="display:flex; justify-content:space-between; align-items:center;">
            <div style="font-size:28px;">🚨</div>
            <div style="font-weight:bold; color:#9a3412;">+0%</div>
        </div>
                ...
                for i in range(lim):
                    ...
                    <div style="padding:12px; background:#f8fafc; border:1px solid #cbd5e1; border-radius:10px; margin-bottom:10px;">
                        <div style="display:flex; justify-content:space-between; margin-bottom:4px;">
                            ...
                            <span class="{sev_style}">{sev_txt}</span>
                        </div>
                        <div style="display:flex; justify-content:space-between; align-items:center;">
