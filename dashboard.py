...
SCROLL_AREA_HEIGHT = 440  # Height of the scrollable content inside the cards

st_autorefresh(interval=REFRESH_SEC * 1000, key="auto_refresh")

# =========================
# CSS: SEPARATED CARDS STYLE
# =========================
st.markdown(
    f"""
<style>
...
/* 2. CARD STYLE (The Visible Outer Box) */
/* This targets the main containers with border=True */
[data-testid="stVerticalBlockBorderWrapper"] {{
    background-color: #ffffff !important;
    border: 1px solid #cbd5e1 !important; /* Clean Grey Border */
...
/* 3. INNER CONTAINER FIX (The Invisible Scroll Box) */
/* Crucial: If a border-wrapper is inside another border-wrapper, remove its styling.
...
.title {{ font-size: 26px; font-weight: 900; margin-bottom: 4px; }}
...
/* 7. Badges & Pills */
.pill-red {{ background: #fee2e2; color: #991b1b; padding: 6px 12px; border-radius: 20px; font-weight: bold; font-size: 13px; }}
.sev-badge {{ padding: 4px 8px; border-radius: 6px; font-size: 11px; font-weight: 800; text-transform: uppercase; }}
...
/* 8. Thumbnails */
.thumb {{ border-radius: 12px; overflow: hidden; border: 1px solid #e2e8f0; position: relative; }}
...
st.markdown(
    ...
    <div class="headerbar">
        <div style="display:flex; justify-content:space-between; align-items:center;">
            ...
            <div class="pill-red">🔔 {new_today} New Alerts</div>
        </div>
    </div>
    """,
    unsafe_allow_html=True,
...
with k1:
    with st.container(border=True):
        ...
        <div style="font-size:36px; font-weight:900; margin-top:5px;">{new_today}</div>
...
with k2:
    with st.container(border=True):
        ...
        <div style="font-size:36px; font-weight:900; margin-top:5px;">{dogs_today}</div>
        <div style="font-size:13px; font-weight:bold; color:#64748b;">TOTAL DOGS</div>
        """, unsafe_allow_html=True)
...
with k3:
    with st.container(border=True):
        ...
        <div style="font-size:36px; font-weight:900; margin-top:5px;">{hp_today}</div>
...
# =========================
# ROW 2: MAIN FEATURES (SEPARATED CARDS)
# =========================
left, mid, right = st.columns([1, 1, 1], gap="medium")

# --- CARD 4: CAMERA FEED ---
with left:
    # Outer "Card" with visible border
    with st.container(border=True):
        st.subheader("📷 Camera Feed")
        st.caption("Live monitoring view")
        
        # Inner scroll area (Border set to FALSE to avoid box-in-box)
        with st.container(height=SCROLL_AREA_HEIGHT, border=False):
            if len(df_sorted) == 0:
                ...
                if img_ok:
                    ...
                    st.markdown("""<div class="thumb" style="height:220px;display:flex;align-items:center;justify-content:center;background:#f8fafc;color:#94a3b8">No Image</div>""", unsafe_allow_html=True)
...
with mid:
    ...
    with st.container(border=True):
        ...
        # Inner scroll area (Border set to FALSE)
        with st.container(height=SCROLL_AREA_HEIGHT, border=False):
            if len(df_sorted) == 0:
                st.info("No alerts.")
            else:
        ...
        with st.container(height=SCROLL_AREA_HEIGHT, border=False):
            ...
            if sel is None:
                ...
                if img_ok:
                    ...
                    st.markdown("""<div class="thumb" style="height:200px;display:flex;align-items:center;justify-content:center;background:#f8fafc;color:#94a3b8">No Image</div>""", unsafe_allow_html=True)
...
with st.container(border=True):
    ...
    if mode == "24 Hours":
        ...
        fig.update_layout(margin=dict(l=10, r=10, t=10, b=10), height=320, paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)')
        ...
        fig.update_layout(barmode="group", margin=dict(l=10, r=10, t=10, b=10), height=320, paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)')
        st.plotly_chart(fig, use_container_width=True)
        ...
        fig.update_layout(margin=dict(l=10, r=10, t=10, b=10), height=320, paper_bgcolor='rgba(0,0,0,0)')
        st.plotly_chart(fig, use_container_width=True)
