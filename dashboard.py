...
REFRESH_MS = 8000          # auto refresh every 8s
TIME_WINDOW_MIN = 10       # severity window

st.set_page_config(
    page_title="Smart City Stray Dog Control System",
    layout="wide"
)

# small, smooth refresh (no big glitch)
...
/* all bordered containers = soft white cards */
...
/* scroll areas inside panels */
.alerts-scroll, .activity-scroll{
  max-height:300px;
  overflow-y:auto;
  margin-top:0.4rem;
...
def generate_dummy_data(n=200):
    """Fallback if Sheet fails – random dog events."""
    now = datetime.utcnow()
...
delta_response     = "-5%"   # fake KPI for now
...
# column that holds image URLs (if any)
image_col = None
...
# severity banner
st.markdown(
    f"<div style='border-radius:1rem;background:#ffffff;border:1px solid #e5e7eb;"
    f"padding:0.75rem 0.9rem;box-shadow:0 1px 3px rgba(15,23,42,0.06);"
    f"border-left:4px solid #f97316;margin-bottom:0.8rem;'>"
...
# ------------------------------------------------------------------
# ROW 2 – three equal light panels
# ------------------------------------------------------------------
...
with row2_col1:
    with st.container(border=True):
        ...
        if alerts.empty:
            ...
            st.markdown(f"<div class='alerts-scroll'>{items_html}</div>", unsafe_allow_html=True)
...
with row2_col3:
    with st.container(border=True):
        ...
        if recent_activity.empty:
            ...
            st.markdown(f"<div class='activity-scroll'>{items_html}</div>", unsafe_allow_html=True)

# ------------------------------------------------------------------
# ROW 3 – Trends + Camera feeds
# ------------------------------------------------------------------
    ...
    if "camera_id" not in df.columns:
        ...
        for _, row in latest_by_cam.iterrows():
            ...
            with st.container(border=True):
                ...
                st.markdown(f"**{loc}**  \n{cam}")
                st.caption(f"Detection {ago_txt} • {conf*100:.0f}% confidence")
