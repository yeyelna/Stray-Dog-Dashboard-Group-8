...
# Fixed visual heights (ROW 2 cards)
ROW2_CARD_HEIGHT_PX = 640
ROW2_HEADER_FOOTER_PAD_PX = 120  # header+caption+spacing inside the card
ALERTS_SCROLL_HEIGHT_PX = max(260, ROW2_CARD_HEIGHT_PX - ROW2_HEADER_FOOTER_PAD_PX)

st_autorefresh(interval=REFRESH_SEC * 1000, key="auto_refresh")

# =========================
# CSS
# - Row 1 & Row 2: use ONLY ONE outer border (st.container(border=True))
# - Remove inner/nested border look
# - Active Alerts: scroll inside fixed height
# =========================
st.markdown(
    f"""
<style>
...
.row-gap{{height:18px}}
/* ---------- OUTER BORDER (ONLY) for border=True containers ---------- */
[data-testid="stVerticalBlockBorderWrapper"]{{
  background:#faf7f2 !important;
  border:2px solid rgba(15,23,42,.28) !important;   /* darker border */
  border-radius:22px !important;
  box-shadow:0 6px 18px rgba(15,23,42,.06) !important;
  padding:16px !important;
}}
/* kill any inner framed look coming from wrapper children */
...
/* IMPORTANT: prevent “extra top blank box” from markdown wrappers spacing */
[data-testid="stVerticalBlockBorderWrapper"] .element-container:empty{{display:none !important}}
/* ---------- ROW 2 fixed height shell (NO extra border) ---------- */
.row2-fixed{{
...
.badge-time{{background:#f1f5f9 !important;color:#0f172a !important;border:1px solid rgba(30,41,59,.10) !important}}
/* Header bar (unchanged) */
.headerbar{{
  background:#ffffff;border:1px solid rgba(30,41,59,.12);
  border-radius:18px;box-shadow:0 6px 18px rgba(15,23,42,.06);
...
.ov-cam{{
  background:rgba(15,23,42,.80);color:#ffffff !important;font-weight:900;font-size:12px;
  padding:6px 10px;border-radius:10px
}}
.ov-det{{
  position:absolute;left:10px;bottom:10px;
  background:#f59e0b;color:#0f172a !important;font-weight:900;font-size:12px;
  padding:6px 10px;border-radius:10px;display:flex;align-items:center;gap:6px
}}
...
# =========================
# ROW 1: KPI (ONE OUTER BORDER ONLY)
# =========================
...
# =========================
# ROW 2: Camera + Active Alerts (scrollable fixed height) + Picture
# - ONE OUTER BORDER ONLY per card
# - ALL 3 cards SAME HEIGHT (via .row2-fixed)
# - Active Alerts scroll inside fixed-height list
# =========================
...
with left:
    with st.container(border=True):
        st.markdown('<div class="row2-fixed">', unsafe_allow_html=True)
        ...
        if len(df_sorted) == 0:
            ...
            if img_ok:
                ...
                st.markdown(
                    """
                    <div class="thumb" style="height:220px;display:flex;align-items:center;justify-content:center;font-weight:900;color:#64748b">
                      No Snapshot URL
                    </div>
                    """,
                    unsafe_allow_html=True,
...
with mid:
    with st.container(border=True):
        ...
        render_alert_list(df_sorted)  # scroll INSIDE fixed-height list
        st.markdown("</div>", unsafe_allow_html=True)
    ...
    with st.container(border=True):
        ...
        if sel is None:
            ...
            if img_ok:
                ...
                st.markdown(
                    """
                    <div style="height:260px;border-radius:16px;border:1px dashed rgba(30,41,59,.18);
...
# =========================
# ROW 3: Trends (UNCHANGED from your last version style)
# =========================
...
with b1:
    st.markdown(
        f"<div style='text-align:center;padding:14px 0 4px 0'><div class='small-muted'>Peak Hour</div><div style='font-weight:900;font-size:22px;color:#0f172a !important'>{peak}</div></div>",
        unsafe_allow_html=True,
    )
with b2:
    st.markdown(
        f"<div style='text-align:center;padding:14px 0 4px 0'><div class='small-muted'>Avg Daily Detections</div><div style='font-weight:900;font-size:22px;color:#0f172a !important'>{avg_daily}</div></div>",
        unsafe_allow_html=True,
    )
st.markdown("</div>", unsafe_allow_html=True)
