...
# =========================
# CSS: HIGH VISIBILITY BORDERS + SHADOWS
# =========================
st.markdown(
    f"""
<style>
...
.stApp{{background:#f7f4ef !important}} /* Beige Background */
...
/* This targets the Main Cards (KPI, Camera, Alerts) */
[data-testid="stVerticalBlockBorderWrapper"]{{
  ...
  margin-bottom: 16px !important; /* Force space below cards */
}}

/* 4. PREVENT DOUBLE BORDERS */
/* If a card is inside another (like scroll area), remove its border/shadow */
...
# =========================
# gap="medium" puts space between the columns so the borders don't touch
k1, k2, k3 = st.columns(3, gap="medium")

with k1:
    with st.container(border=True): # This container creates the Border + Shadow
        st.markdown(
...
# =========================
# ROW 2: MAIN FEATURE CARDS (SEPARATED)
# =========================
left, mid, right = st.columns([1.05, 0.95, 1.05], gap="medium")

# --- LEFT (Camera) ---
with left:
    with st.container(border=True): # Outer Border & Shadow
        st.subheader("📷 Camera Feeds & Snapshots")
        st.caption("Latest detection (single feed)")
        
        # Inner scroll (NO BORDER set in Python to avoid nesting)
